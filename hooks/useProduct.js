  ...
  const addProduct = async (productData) => {
    ...
    const { data, error } = await supabaseClient
      ...
      .insert([
        {
          ...productData,
          color: productData.color ? JSON.stringify(productData.color) : "[]", // Store colors as JSON array
          size: productData.size ? JSON.stringify(productData.size) : "[]", // Store sizes as JSON array
        },
      ]);
